...
Section \ref{sec:app-most-famous} compares \citet{fama1992cross}, \citet{jegadeesh1993returns}, and \citet{banz1981relationship} with data mining. Section \ref{sec:conclusion} concludes. Robustness is in the Appendix.
...
\subsection{Peer-Reviewed Predictor Data and Benchmarks} \label{sec:pubvs-data-pub}
...
Some tests use variations on this baseline benchmark. One variation filters the data-mined strategies to those that match the published signals in terms of in-sample performance: sample mean returns are within 30\% and t-stats within 10\% of those of the published signals. This performance match is applied in place of the $|t|>2.0$ screen. Another variation drops data-mined strategies that have returns that are more than 10\% correlated with the published signals' returns.
...
One might also worry that we should not compare published and data-mined strategies that differ strongly in in-sample performance. Panel (c) addresses this concern by requiring data-mined strategies to have at least 60 original-sample months and to match the published signals within 10\% on t-statistics and 30\% on mean returns (Section \ref{sec:pubvs-bench}). This filter has little effect on our main result.
...
% further robustness
Section \ref{sec:hetero} shows additional robustness. Importantly, it shows that predictors with theoretical support also fail to outperform data mining. Indeed, if anything, predictors with that \emph{lack} a theoretical foundation show the most promise for outperformance. [tbc: do we keep the most reknown section?]

\subsection{Post-Sample Outperformance by Discipline and Journal Ranking}\label{sec:hetero-discipline}

\hl{tbc: integrate into main text, consider moving to end}

Table \ref{tab:hetero-byjournal} examines how discipline and journal ranking affect outperformance. The top panel shows that predictor-papers in finance journals have stronger post-sample performance compared to accounting journals. Finance predictor-papers produce 59 to 75 bps per month post-sample, compared to 43 to 48 bps per month for accounting journals.
...
This performance is less impressive when compared to data mining, however. Predictor-papers in finance journals outperform data mining by between 8 and 13 bps per month.  Predictor-papers in accounting journals underperform data mining, regardless of the performance metric.

The top 3 finance journals (Journal of Finance, Journal of Financial Economics, Review of Financial Studies) perform a bit better than finance journals overall. Nevertheless, the outperformance relative to data mining is small. Predictor-papers in these top journals outperform data mining by 8 to 16 bps per month, depending on the performance metric. Given the normalization, this means top finance journals retain an additional 8 to 16 percentage points of their original-sample performance, relative to simple data mining.

\providecommand{\Cor}{\text{Cor}}

% Section 4
\section{Learning from Academics or Learning from the Data?}\label{sec:mp}\label{sec:learn}
...
% Section 4.1
\subsection{A Model of Learning and Data Mining Bias}\label{sec:mp,model}\label{sec:model}
...
% Section 4.3
\subsection{Separating the Two Interpretations}\label{sec:learn,interp2}
...
% Section 4.4
\subsection{Regression Evidence}\label{sec:mp,regs}
...
Section \ref{sec:hetero-discipline} showed that journal ranking has relatively little effect. But perhaps one needs to go beyond journal ranking, and focus on the very best research, to find notable outperformance relative to data mining.
